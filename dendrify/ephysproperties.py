...
class EphysProperties(object):
    ...
    def __init__(self, tag=None, length=None, diameter=None, cm=None,
    ...
    def area(self):
        ...
        try:
            ...
            print(("ERROR: Missing Parameters ('length' or 'diameter')\n"
                  f"Could not calculate the area of <{self.tag}>, "
    ...
    def capacitance(self):
        ...
        try:
            ...
            print(("ERROR: Missing Parameters ('cm')\n"
                  f"Could not calculate the capacitance of <{self.tag}>, "
                   "returned None instead"))
    ...
    def g_leakage(self):
        ...
        try:
            ...
            print(("ERROR: Missing Parameters ('gl')\n"
                  f"Could not calculate the g_leakage of <{self.tag}>, "
                   "returned None instead"))
    ...
    def parameters(self):
        ...
        if self.v_rest:
            d[f"EL_{self.tag}"] = self.v_rest
        else:
            print(f"ERROR: Could not resolve 'EL_{self.tag}'\n")
            error = True

        if self.capacitance:
            d[f"C_{self.tag}"] = self.capacitance
        else:
            print(f"Could not resolve 'C_{self.tag}'\n")
            error = True

        if self.g_leakage:
            d[f"gL_{self.tag}"] = self.g_leakage
        else:
            print(f"Could not resolve 'gL_{self.tag}'")
            error = True

        if error:
            print("\nWARNING: One or more parameters are "
                  f"missing for '{self.tag}' !!!\n")
        return d
    ...
    def g_cylinder(self):
        ...
        try:
            ...
            print(("ERROR: Missing Parameters <length / diameter / r_axial>\n"
                  f"Could not calculate the g_cylinder of <{self.tag}>, "
                   "returned None instead."))
        else:
            return 1/ri
    ...
    def g_couple(comp1, comp2):
        ...
        try:
            ...
            print(("ERROR: Missing Parameters <length / diameter / r_axial>\n"
                   f"Could not calculate the g_couple of <{comp1.tag}> "
                   f"& <{comp2.tag}>, returned None instead."))
        else:
            return 1/ri
